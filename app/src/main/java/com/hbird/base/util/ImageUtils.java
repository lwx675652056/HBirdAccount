...
//        aaa.setImageBitmap(cachebmp);//直接展示转化的bitmap
    ...
    public static File viewSaveTo2Image(View view,String child) {
        cachebmp = loadBitmapFromView(view);
        FileOutputStream fos;
        File file = null;
        try {
            ...
            if (isHasSDCard) {
                ...
                File sdRoot = Environment.getExternalStorageDirectory();
                //Log.e("ssh",sdRoot.toString());
                file = new File(sdRoot, child+".png");
                fos = new FileOutputStream(file);
            } else
                throw new Exception("创建文件失败!");
            //压缩图片 30 是压缩率，表示压缩70%; 如果不压缩是100，表示压缩率为0
    ...
    public static Bitmap getCachebmp(View view){
        cachebmp = loadBitmapFromView(view);
        return cachebmp;
    }
...
//        c.drawColor(Color.WHITE);
    ...
    //保存在本地并一键分享
    public static String share2Pic(Bitmap cachebmp, String child) {
