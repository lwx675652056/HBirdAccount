...
//            selectView.findViewById(R.id.delete_img).setVisibility(VISIBLE);
    ...
    private void swapItems(int x, int y)
    {
        ...
        Log.e("Liul",endPosition+"---"+currentPosition);


        //判断是否要进行位置交换
        if (endPosition != INVALID_POSITION && endPosition != currentPosition )//&& endPosition!=0 && endPosition!=1 && endPosition!=2 && endPosition!=3)
        {
            isSwap = true;
            isEdit = false;
            resumeView();
