...
public class LineChartEntity extends BaseChartEntity<Entry> {
    ...
    public LineChartEntity(LineChart lineChart, List<Entry>[] entries, String[] labels, int[] chartColor, int valueColor, float textSize) {
        super(lineChart, entries, labels, chartColor, valueColor, textSize);
    }

    public LineChartEntity(LineChart lineChart, List<Entry>[] entries, String[] labels, boolean[] hasDotted, int[] chartColor, int valueColor, float textSize) {
...
//        mChart.getXAxis().enableGridDashedLine(20f, 20f, 0f);
        ...
        l.setEnabled(false);//取消表头（之前显示2018年份 和 需要对比的年份 现在删除不展示）
    }
...
//                    lineDataSet[index].setFillColor(ColorTemplate.colorWithAlpha(Color.YELLOW, 200));
            ...
            // set data
            mChart.setData(data);
            mChart.animateX(1000, Easing.EasingOption.EaseInOutQuad);
        }
    }
    ...
    public void drawCircle(boolean draw) {
        ...
        /* int count = sets.get(1).getEntryCount();
       for(int i=0;i<count;i++){
            float y = sets.get(1).getEntryForIndex(i).getY();
            if(y==0.0){
