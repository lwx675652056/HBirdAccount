 ...
 * 图表柱状图 倒着展示
 * @author liul
 * created by 2018-08-09
 */
public class IndicatorChartDownProgressBar extends ProgressBar {

    private final String TAG = IndicatorChartDownProgressBar.class.getSimpleName();
    ...
    public IndicatorChartDownProgressBar(Context context) {
        super(context);
        init(null, 0);
    }

    public IndicatorChartDownProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public IndicatorChartDownProgressBar(Context context, AttributeSet attrs, int defStyle) {
...
//        }
        ...
        if (indicatorProgressBar != null && indicatorProgressBar instanceof LayerDrawable) {
            ...
            dx = indicator_progress.getBounds().bottom;
        } else if (indicatorProgressBar != null) {
            dx = indicatorProgressBar.getBounds().bottom;
        }
...
//            Log.i(TAG,PX2DPUtil.dp2px(getContext(), 25)+"==进度条的长度==="+dx);
    ...
    private void updateHeightProgressBar() {
        ...
        if (progressDrawable != null && progressDrawable instanceof LayerDrawable) {
            ...
            //指示器drawable的宽度
            final int heights = rect.bottom - rect.top;//+ mXOffsetAbove + mWidthAbove
            if (indicator_progress != null) {
                Rect indicatorProgressBounds = indicator_progress.getBounds();
                indicatorProgressBounds.bottom = indicatorProgressBounds.top + (int) (heights * scale);
                //设置开始进度时的初始宽度
                indicatorProgressBounds.right = getMeasuredWidth();
                //设置进度条的drawable的宽高
                indicator_progress.setBounds(indicatorProgressBounds);

                int progressHeight = indicatorProgressBounds.bottom;
