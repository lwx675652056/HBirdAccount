 ...
 * description: 系统收入类目表 数据表管理者 for GreenDao
 * DevRing中提供了GreenTableManager(基本的数据表管理者)，继承它然后实现getDao方法，将GreenDao自动生成的对应XXXDao返回即可。
 */

public class HbirdIncomeTypeManager extends GreenTableManager<HbirdIncomeType,Void> {
    private DaoSession mDaoSession;

    public HbirdIncomeTypeManager(DaoSession daoSession) {
        mDaoSession = daoSession;
    }
    @Override
    public AbstractDao<HbirdIncomeType, Void> getDao() {
        return mDaoSession.getHbirdIncomeTypeDao();
    }
}
